...
For each fault scenario in FAULT_MATRIX, this test:
    ...
    2. Runs the diagnosis pipeline against that database using the same
       evidence-gathering path the production /diagnose endpoint uses.
    3. Asserts that primary_root_cause matches at least one of the labelled
       fault types for that scenario.

Success criterion: passes on ≥ 6 of 8 canonical fault scenarios. Below that
threshold the supervisor tiebreak or a specialist branch has regressed and
the diagnosis pipeline should not ship.
...
async def test_diagnosis_matches_expected_root_cause(scenario_name: str, fault_lab_available: bool) -> None:
    """Apply a fault, run diagnosis, assert the primary root cause matches."""
